...
def get_all_balances(current_user_id):
    ...
    faseeh_id = next((uid for uid, name in users.items() if name.lower() == 'faseeh'), None)
    sudhakaran_id = next((uid for uid, name in users.items() if name.lower() == 'sudhakaran'), None)
    hari_id = next((uid for uid, name in users.items() if name.lower() == 'hari'), None)
