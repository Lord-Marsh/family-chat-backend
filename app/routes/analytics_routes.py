    ...
    try:
        ...
        for s in top_expenses:
            top_expenses_mapped.append({
                ...
                'date': s.get('createdAt').isoformat() if isinstance(s.get('createdAt'), datetime) else str(s.get('createdAt'))
            })
        ...
        import traceback
        return jsonify({'message': f"Server Error: {str(e)}", 'trace': traceback.format_exc()}), 500
